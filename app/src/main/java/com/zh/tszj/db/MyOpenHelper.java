...
public class MyOpenHelper extends SQLiteOpenHelper implements IOpenHelper {
    ...
    public void save(Object obj) {
        ...
        //具体实现保存数据方法
        save(obj, table, getWritableDatabase(), true);
    }
     ...
     *
     * @param obj
     * @param isReplace 是否替换  等同与更新
     */
    public void save(Object obj, Boolean isReplace) {
        //获取类类型
        Class<?> table = obj.getClass();
        //创建对应的表
        createTableIfNotExists(table);
        //具体实现保存数据方法
        save(obj, table, getWritableDatabase(), isReplace);
    }
     ...
     * @param db    操作数据库
     */
    private void save(Object obj, Class<?> table, SQLiteDatabase db, Boolean isReplace) throws NullPointerException {
        //将一个对象中的所有字段添加到该数据集中
        ...
        try {
            ...
            if (isReplace)
                ...
                db.insert(table.getName().replaceAll("\\.", "_"), null, contentValues);
     ...
     */
    @Override
    public void saveAll(Collection collection) throws NullPointerException {
        //如果集合为空直接不需要操作
    ...
    public void saveAll(Collection collection,boolean b) throws NullPointerException {
        //如果集合为空直接不需要操作
        if (collection.isEmpty()) {
            return;
        }
     ...
     */
    @Override
    public <T> List<T> queryAll(Class<T> table, String orderBy) throws NullPointerException {
     ...
     */
    @Override
    public <T> List<T> queryAll(Class<T> table, String orderBy, int limit) throws NullPointerException {
        //如上雷同，不做介绍
        if (!isTableExists(table)) {
            return null;
        }
     ...
     */
    @Override
    public <T> T queryById(Class<T> table, Object id) throws NullPointerException {
        Field idField = null;
        //获取属性id
    ...
    public <T> List<T> queryByKey(Class<T> table, String key, String value) {
        try {
            SQLiteDatabase db = getReadableDatabase();
            Cursor cursor = db.query(table.getName().replaceAll("\\.", "_"),
                    null, key + " = ?", new String[]{String.valueOf(value)}, null, null, null);
            List<T> list = initList(table, cursor);
            return list;
     ...
     * @return
     */
    public <T> List<T> queryBySql(Class<T> table, String sql, String[] value) {
        try {
            SQLiteDatabase db = getReadableDatabase();
            String tabName = table.getName().replaceAll("\\.", "_");
            Cursor cursor = db.rawQuery("SELECT * FROM " + tabName + " where " + sql, value);
            List<T> list = initList(table, cursor);
            return list;
        } catch (Exception exception) {
            exception.printStackTrace();
     ...
     * @return
     */
    private <T> List<T> initList(Class<T> table, Cursor cursor) throws NullPointerException {
     ...
     * @param table
     */
    private void createTableIfNotExists(Class table) throws NullPointerException {
        if (!isTableExists(table)) {
            SQLiteDatabase db = getWritableDatabase();
     ...
     * @return
     */
    private Field getFieldId(Class table) throws NullPointerException {
        Field fieldId = null;
        try {
